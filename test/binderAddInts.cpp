...
static void client(void)
{
    ...
    for (unsigned int iter = 0; iter < options.iterations; iter++) {
        ...
        if (options.payloadSize < sizeof(system_time_test_t)) {
            ...
            uint64_t timetick;
            //printf("send timestamp:%ld\n", std::chrono::duration_cast<std::chrono::nanoseconds>(d).count());
            memcpy(&timetick, &d, sizeof(d));
            send.writeInt32(strlen(buf));
            send.writeUint64(timetick);
    ...
    cout << "Time per iteration min: " << min
        << " avg: " << (total / options.iterations) << "s"
        << " max: " << max << "s"
        << endl;
}
...
status_t AddIntsService::onTransact(uint32_t code, const Parcel &data,
    ...
    case ADD_INTS:
        if (options.payloadSize == 0) {
            ...
            typedef std::chrono::system_clock::duration system_time_test_t;
            val1 = data.readInt32();
            static system_time_test_t tickcnt;
            uint64_t timetick = data.readUint64();
            const char *str = data.readCString();

            system_time_test_t count = *(reinterpret_cast<std::chrono::system_clock::duration*>(&timetick));
            ...
            #if 0
            printf("get stamp:%ld, currnt:%ld, timestamp:%ld\n", std::chrono::duration_cast<std::chrono::nanoseconds>(count).count()
            , std::chrono::duration_cast<std::chrono::nanoseconds>(d).count()
            , std::chrono::duration_cast<std::chrono::nanoseconds>(d - count).count());
            #endif


            if (i >= calPerCnt)
            {
                printf("BIG_PAYLOAD_SIZE_TEST size: %dByte  times:%d stamp: %ld\n", options.payloadSize, calPerCnt,  std::chrono::duration_cast<std::chrono::nanoseconds>(tickcnt).count()/ calPerCnt);
                i = 0;
                tickcnt = std::chrono::system_clock::duration();
            }
